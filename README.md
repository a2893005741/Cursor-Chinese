...
- 包含 Cursor / VS Code 简体中文语言包融合版本。
- 包含 Cursor 主界面、Agent、Composer、设置页等界面的中文化补丁。
- 提供一键安装脚本，安装前会自动备份被覆盖的文件。
...
| `安装汉化包.ps1` | 正式安装脚本，负责查找 Cursor、备份旧文件、复制汉化文件。 |
...
| `ms-ceintl.vscode-language-pack-zh-hans-1.121.2026052106-cursor105-fused.vsix` | 融合后的简体中文语言包。 |
| `install-backups/` | 安装时自动生成的备份目录。首次下载时可能不存在。 |

## 安全说明

- 安装前请先完全关闭 Cursor。
- 安装脚本会先备份将要覆盖的文件，再复制汉化文件。
...
- 融合包文件：`ms-ceintl.vscode-language-pack-zh-hans-1.121.2026052106-cursor105-fused.vsix`
